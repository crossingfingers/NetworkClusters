...
void mult_array(const struct _spmat *A, const double *vec, double *result) {
    array *sparray = (array *) A->private;
    int *rowPtr = sparray->rowptr, groupSize = A->n;
    register int *cols;
    register int i;
 ...
 * @param F : an array containing the sum of values for each column
 * @return the shifting value
 * */
double arrayShifting(spmat *A, const double *F) {
    double max = 0;
    register double sum;
    register int val;
